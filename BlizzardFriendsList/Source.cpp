...
int main(int argc, char** argv)
{
	...
	printf("          Battletag     |     Name         |  MemAddr  |  Status |   UnknownTimestamp  |      Last Seen      |  SessId  |        Friend Note\n");
	for (auto it = users.begin(); it != users.end(); ++it)
	{
		...
		// Display timestamps
		int64_t lastSeenTimestamp = *(int64_t*)(accountBuffer + 0x78);
		int64_t friendTimestamp = *(int64_t*)(accountBuffer + 0x80);
		printdate(friendTimestamp);
		printdate(lastSeenTimestamp);
